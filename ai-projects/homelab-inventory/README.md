...
The inventory came back exactly as expected. Appended to it was a findings section I never requested. The agent had been reading config files to build a hardware list, recognised what it was seeing, and documented it.
...
| 🟠 HIGH | ISP PPPoE credentials in plaintext, router compromise means ISP account compromise | OpenWrt router |
...
**The missing onboot flag** is a quieter finding but would have caused real pain. Eight services on one LXC, including Postgres and Redis with live data, would have silently disappeared on the next Proxmox reboot. One command to fix. The agent provided it.
...
- Spawn a privileged container mounting the host root filesystem, with full read and write access to everything on that host
...
**On the bigger picture:** This does not require expensive tooling or specialist knowledge. SSH access and a Claude subscription. The agent reads the environment, identifies what matters, and explains the implications without being asked. That is useful for anyone doing legitimate audit work. It is the same capability regardless of intent.

**Two questions this raises:**

1. Security auditing has traditionally required specialist skills most people do not have. If AI can surface critical misconfigurations from a documentation request, what does that mean for how smaller organisations approach security?
...
The audit worked. Almost too well. It also exposed some serious security vulnerabilities.

That raised an obvious question. If a documentation request produced this, what does a deliberate security engagement produce?

The next project sets out to answer that. A purpose-built isolated lab, two AI agents with opposing briefs. One defends, one attacks. Same environment, same vulnerabilities, two different perspectives. Then remediation. Then a retest.

If the first project was an accident, the next one is intentional.
